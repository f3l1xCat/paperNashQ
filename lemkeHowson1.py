...
def createTableaux(m1, m2):
    ...
    if m1.getNumRows() != m2.getNumRows() or m1.getNumCols() != m2.getNumCols():
        raise ValueError('Selected matrices does not have the same number ' +                'of rows and columns')
...
def makePivotingStep(t, p1SCount, ebVar):
    ...
    for i in getRowNums(ebVar):
        if t.getItem(i, varToCol(ebVar)) != 0:
            for j in range(2, t.getNumCols() + 1):
                newVal = t.getItem(i, j) + t.getItem(i, varToCol(ebVar)) *                        t.getItem(lbVarRow, j)
    ...
    for i in range(0, 2):
        for j in range(0, len(eq[i])):
            if not isinstance(eq[i][j], rational.Rational):
                raise ValueError('Selected equilibrium contains a ' +                    'non-rational number.')
...
#         # set it to zero instead
#         eqs[abs(strat) - 1] = rational.Rational(0) if (strat < 0 or prob < 0) else prob
#     print(rational.Rational(1), eqs[3:])
...
#     print('m1.getNumRows:',m1.getNumRows(), 'm1.getNumCols:', m1.getNumCols())
    randomRow = random.randrange(0, m1.getNumRows() - 1)
    randomCol = random.randrange(0, m1.getNumCols() - 1)
    randomRowlist=[]
    randomCollist=[]
    for i in range(m1.getNumRows()):
        if i == randomRow:
            randomRowlist.append(rational.Rational(1))
        else:
            randomRowlist.append(rational.Rational(0))
    for i in range(m1.getNumCols()):
        if i == randomRow:
            randomCollist.append(rational.Rational(1))
        else:
            randomCollist.append(rational.Rational(0))
    ...
    while abs(leftBasisVar) != initBasisVar:
        ...
        count += 1
        if count > 10:
#             print('not equilibrium: ', (tuple(randomRowlist), tuple(randomCollist)))
            return (tuple(randomRowlist), tuple(randomCollist))
