...
def gridGameOne(action_0, action_1, currentState, count, agent_0, agent_1):
    ...
    theta_bar = (K*(V-nextIndex_0)+beta*sigma*(phi*nextIndex_0-t))/(K*(delta+beta*M))
    theta_hat = (t+nextIndex_1+delta-V)/delta
    d_a=(phi*nextIndex_0-t)/K
    U_uber = V-delta*theta_bar-beta*(theta_bar*M-sigma*d_a)-nextIndex_0
    U_buget = V-delta*(1-theta_hat)-t-nextIndex_1
...
#         print(currentState, (nextIndex_0, nextIndex_1), testlemkeHowson_agent0, testlemkeHowson_agent1)
    ...
    if (nextIndex_0 < 0 or nextIndex_1 < 0) or (nextIndex_0 > WORLD_HEIGHT or nextIndex_1 > WORLD_WIDTH):
        reward_0 = -100
        reward_1 = -100
        if testlemkeHowson_agent0==False or testlemkeHowson_agent1==False:
            reward_0=0
...
#         U_buget = V-delta*(1-theta)-t-nextIndex_1
        r1 = (1-phi)*nextIndex_0*theta_bar*M
        if eta >= (1-theta_hat)*M:
            r2 = nextIndex_1*(1-theta_hat)*M-c*eta
        else:
